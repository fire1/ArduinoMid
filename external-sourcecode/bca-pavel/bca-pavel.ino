...
void setup() {
    ...
    if (value == LOW) {
        ...
        lcd.setCursor(0, 1);
        lcd.print(
                "|----[recovery]----|");   //забраните дърпа данни от ELM327 чрез натискане на бутона при писане Connecting...
        tone(13, 3000, 500);
        delay(2000);
...
void loop() {
    ...
    if (off2) {
        ...
        if ((t1 == 60) or (t2 ==
                           10)) { // чтение температуры и вольтажа каждые 100 циклов работы (примерно 30 секунд), первый раз чтение выполниться через примерно 5 секунд со старта
            Tmp();
            Voltage();
            LongTerm();
            t1 = 0;
    ...
    if (rpm_var >
        engine_on_rpm) {     // если обороты больше engine_on_rpm то считаем расход и расстояние (машина заведена)

        FuelSystemStatus();

        Rnd(); // вытягиваем информацию о нагрузка двигателя, если нету такго параметра в ЭБУ то закомментировать строчку символами //

        if (raschet_nagruzka_dvigatelya_var >
            5.0) {  // Проверка торможения двигателем по нагрузке двигателя, если информации о нагрузке в ЭБУ нету то поменять на ((fss_val==2) or (speed_var>70))
            ...
            FuelFlowGramsPerSecond = double(MAF / AirFuelRatio) *
                                     ls_term_val;   // Получаем расход грамм бензина в секунду в соотношении 14,7 воздуха/к 1 литра бензина, корректировка ls_term_val
            FuelFlowLitersPerSecond =
                    FuelFlowGramsPerSecond / FuelDensityGramsPerLiter;  // Переводим граммы бензина в литры
        ...
        time = (double(time_new - time_old) / 1000.0) *
               tcorrect;     // прошло время с последнего расчета скорости, расхода  - в сек
        if (time > 10) { time = 0; }
        ...
        if (((speed_var > 1) and (speed_var < 10) and ((time_new - time_old_gurnal) > 30000)) or
            ((speed_var == 0) and ((time_new - time_old_gurnal) > 10000))) {
            ...
            EEPROM_float_write(111,
                               (odometr_eeprom)); // записываем в энергонезависимую память журнала расстояние каждые 5 секунд... в памяти занимаются ячейкм 111,112,113,114
    ...
    if (odometr > 0) {
        LP100 = (benz_potracheno / odometr) *
                100.0;                                          //расход бензина на 100 км (в литрах)
    }
...
void Rpm(void) {
    ...
    getResponse();
    getResponse();
    if ((strtol(&rxData[0], 0, 16) == 65) and (strtol(&rxData[3], 0, 16) ==
                                               12)) {    //запрос приходит от ЭБУ в виде "41 0С XX XX", где XX XX это обороты, а 41 0С это проверка, где 0С это запрос оборотов
        rpm_var = double((strtol(&rxData[6], 0, 16) * 256) + strtol(&rxData[9], 0, 16)) /
                  4.0;  // но мы проверяем в десятичном формате, а запрос в 16чном, итого 41 это 65, а 0С - 12.
        if (rpm_var < 399) {
            rpm_var = 0;
        }
    }
...
void Rnd(void) {
    ...
    if ((strtol(&rxData[0], 0, 16) == 65) and (strtol(&rxData[3], 0, 16) == 4)) {
        ...
        rnd_error++;; // считаем кол-во ошибок чтения
    }
}
...
void Dts(void) {
    ...
    if ((strtol(&rxData[0], 0, 16) == 65) and (strtol(&rxData[3], 0, 16) == 10)) {
        ...
        dts_error++;; // считаем кол-во ошибок чтения
    }
}
...
void Uoz(void) {
    ...
    if ((strtol(&rxData[0], 0, 16) == 65) and (strtol(&rxData[3], 0, 16) == 14)) {
        ...
        uoz_error++;; // считаем кол-во ошибок чтения
    }
}
...
void Maf(void) {
    ...
    if ((strtol(&rxData[0], 0, 16) == 65) and (strtol(&rxData[3], 0, 16) == 16)) {
        ...
        maf_error++;; // считаем кол-во ошибок чтения
    }
}
...
void Pdz(void) {
    ...
    if ((strtol(&rxData[0], 0, 16) == 65) and (strtol(&rxData[3], 0, 16) == 17)) {
        ...
        pdz_error++;; // считаем кол-во ошибок чтения
    }
}
...
void Uroventopliva(void) {
    ...
    if ((strtol(&rxData[0], 0, 16) == 65) and (strtol(&rxData[3], 0, 16) == 47)) {
        ...
        ut_error++;; // считаем кол-во ошибок чтения
    }
}
...
void Tempmasla(void) {
    ...
    if ((strtol(&rxData[0], 0, 16) == 65) and (strtol(&rxData[3], 0, 16) == 92)) {
        ...
        tm_error++;; // считаем кол-во ошибок чтения
    }
}
